...
namespace CustomCursor
{
    public class CursorManager
    {
        private class CursorDataFile
        {
            ...
            public List<ScaleEntry> scales = new List<ScaleEntry>();
        }
        ...
        private List<string> scaleNameList = new List<string>();
        private Dictionary<string, ScaleItem> registeredScales = new Dictionary<string, ScaleItem>();
        ...
        public void SetCustomCursorIsOn(bool value) => customCursorIsOn = value;
        public void SetPlayerOneCursor(Texture2D value) => playerOneCursor = value;
        public void SetPlayerOneCursorModulation(Color value) => playerOneCursorModulation = value;
        public void SetPlayerOneCursorScale(float value) => playerOneCursorScale = value;
        public void SetPlayerTwoCursor(Texture2D value) => playerTwoCursor = value;
        public void SetPlayerTwoCursorModulation(Color value) => playerTwoCursorModulation = value;
        public void SetPlayerTwoCursorScale(float value) => playerTwoCursorScale = value;
        ...
        internal void LoadAllCursorData()
        {
            ...
            registeredScales[defaultScaleName] = new ScaleItem { name = defaultScaleName, scale = 1f };
            scaleNameList.Add(defaultScaleName);
            ...
            foreach (string path in files)
            {
                ...
                if (data.scales != null)
                {
                    ...
                    {
                        string scaleName = scale.scale.ToString();
                        if (!registeredScales.ContainsKey(scaleName))
                        {
                            registeredScales[scaleName] = new ScaleItem { name = scale.scale.ToString(), scale = scale.scale };
                            scaleNameList.Add(scaleName);
                        }
                    }
                }
            }

            Debug.Log($"Loaded cursors: {cursorNameList.Count}, modulations: {modulationNameList.Count}, sizes: {scaleNameList.Count}");
        }
        ...
        private void UpdatePlayerOneCursorScale(string value = null)
        {
            ...
            if (registeredScales.TryGetValue(value, out ScaleItem scaleItem))
                SetPlayerOneCursorScale(scaleItem.scale);
            else
        ...
        private void UpdatePlayerTwoCursorScale(string value = null)
        {
            ...
            if (registeredScales.TryGetValue(value, out ScaleItem scaleItem))
                SetPlayerTwoCursorScale(scaleItem.scale);
            else
        ...
        internal void InitializeGunfig()
        {
            ...
            gunfig.AddScrollBox(key: PlayerOneCursorScaleStr, label: GameManager.Options.CurrentLanguage == StringTableManager.GungeonSupportedLanguages.CHINESE ?
                "1P光标缩放" :
                PlayerOneCursorScaleStr, options: scaleNameList, updateType: Gunfig.Update.Immediate,
            ...
            gunfig.AddScrollBox(key: PlayerTwoCursorScaleStr, label: GameManager.Options.CurrentLanguage == StringTableManager.GungeonSupportedLanguages.CHINESE ?
                "1P光标缩放" :
                PlayerTwoCursorScaleStr, options: scaleNameList, updateType: Gunfig.Update.Immediate,
